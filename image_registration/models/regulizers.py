...
def compute_hyper_elastic_loss(input_coords, output, batch_size=None, alpha_l=1, alpha_a=1, alpha_v=1):
    ...
    cofactors = torch.zeros(batch_size, 3, 3).to(input_coords.device)
    ...
    volume_loss = torch.mul(
        torch.pow(volume_loss - 1, 4), torch.pow(volume_loss, -2)
    )
...
def compute_bending_energy(input_coords, output, batch_size=None):
    ...
    dx_xyz = torch.zeros(input_coords.shape[0], 3, 3).to(input_coords.device)
    dy_xyz = torch.zeros(input_coords.shape[0], 3, 3).to(input_coords.device)
    dz_xyz = torch.zeros(input_coords.shape[0], 3, 3).to(input_coords.device)
    for i in range(3):
...
def compute_jacobian_loss(input_coords, output, batch_size=None):
    ...
    det = torch.det(jac)
    loss = torch.linalg.norm(det - 1, 1)
...
def compute_jacobian_matrix(input_coords, output, add_identity=True):
    ...
    jacobian_matrix = torch.zeros(input_coords.shape[0], 3, 3).to(
        input_coords.device
    )
    for i in range(3):
